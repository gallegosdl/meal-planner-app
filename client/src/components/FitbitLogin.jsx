...
function FitbitLogin() {
  ...
  const handleFitbitData = async (data) => {
    try {
      ...
      // Log all Fitbit data to console
      console.log('All Fitbit Data:', {
        profile,
        tokens,
        allData
      });
  ...
  const handleFitbitLogin = async () => {
    try {
      ...
      // Poll popup to detect when it's closed
      const pollTimer = setInterval(() => {
        if (!popup || popup.closed) {
          clearInterval(pollTimer);
          // Only show cancellation message if we haven't received data
          if (!fitbitData) {
            setLoading(false);
  ...
  const formatHeight = (heightCm) => {
    ...
    const totalInches = Math.round(heightCm / 2.54);  // 1 inch = 2.54 cm
    
    // Then convert inches to feet and remaining inches
  ...
  if (fitbitData) {
    const height = formatHeight(fitbitData.height);
    ...
    return (
      <div className="bg-[#252B3B]/50 backdrop-blur-sm rounded-2xl p-6 border border-[#ffffff0f] h-full flex flex-col">
        <div className="text-white">
          <div className="flex justify-between items-center mb-4">
            <div>
              <h3 className="text-lg font-semibold text-green-400">
                Fitbit Connected - <span className="text-white font-normal">{fitbitData.displayName || fitbitData.fullName}</span>
                {fitbitData.age && <span className="text-white font-normal"> ({fitbitData.age} years)</span>}
              </h3>
            ...
            <div className="flex items-end gap-6">
              <div className="text-right">
                ...
                <p className="text-base">{new Date(fitbitData.memberSince).toLocaleDateString()}</p>
              </div>
              <button 
          ...
          <div className="mb-3">
            {fitbitData.allData?.activities?.daily?.summary?.steps && (
              <div>
                <p className="text-sm text-gray-400">Today's Steps</p>
                <p className="text-xl">{Number(fitbitData.allData.activities.daily.summary.steps).toLocaleString()}</p>
              </div>
            )}
          </div>
          ...
          <div className="mt-3 pt-4 border-t border-[#ffffff1a]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {fitbitData.allData?.activities?.daily?.activities?.map((activity, index) => {
